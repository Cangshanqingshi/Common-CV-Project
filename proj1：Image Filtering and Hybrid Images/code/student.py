...
def my_imfilter(image, filter):
    ...
    #####################################################################################################
    # 先判定滤波器有没有维度数值是偶数的
    assert len(filter.shape) == 2, "滤波器维数不为2"
    assert (filter.shape[0] * filter.shape[1]) % 2 != 0, "滤波器不能有维度的值为偶数"

    # 由于要输出原尺寸图像，因此先对图像进行padding操作，再滤波
    # 计算填充相关数值
    pad_x = (filter.shape[0] - 1) // 2
    pad_y = (filter.shape[1] - 1) // 2
    # 填充
    image_pad = np.pad(image, ((pad_x, pad_x), (pad_y, pad_y), (0, 0)), 'constant')
    ...
    for k in range(image.shape[2]):
        for i in range(filtered_image_height):
            for j in range(filtered_image_width):
                filtered_image[i, j, k] = np.sum(np.multiply(image_pad[i:i + filter.shape[0], j:j + filter.shape[1], k],
                                                             filter))
    #####################################################################################################
    #                                               End                                                 #
    #####################################################################################################
